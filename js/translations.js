...
var FAQ_TRANSLATIONS = {
  1:{
    ...
    a:{
      en:`Handle the assessment in accordance with Appendix 1 of the Internal Regulation No. 3:
...
Countries with high transshipment risk to Russia refer to countries announced by U.S. FinCEN or domestic competent authorities, including association notices; see Appendix 3 of the Internal Regulation No. 8.
...
- 輸出信用状（L/C）
- 輸出取立取引（D/P、D/A）
ロシア向け積替高リスク国とは、米国FinCENまたは国内主管機関（公会通知を含む）が公告した国を指します（「内規八」別紙3参照）。
...
function translateReferenceText(text, lang){
  ...
  const documentNames = {
    "內規三":{
      en:"Internal Regulation No. 3",
      ja:"内規三"
    },
    "內規二":{
      en:"Internal Regulation No. 2",
      ja:"内規二"
    },
    "內規七":{
      en:"Internal Regulation No. 7",
      ja:"内規七"
    },
    "內規八":{
      en:"Internal Regulation No. 8",
      ja:"内規八"
    }
  };
  const terms = lang === "ja"
  ...
  if(lang === "en" || lang === "ja"){
    ...
    if(lang === "ja"){
      const internalRegulationCodes = {
        "内規2":"内規二",
        "内規3":"内規三",
        "内規7":"内規七",
        "内規8":"内規八"
      };
      Object.entries(internalRegulationCodes).forEach(([numericCode, kanjiCode]) => {
        next = next.replaceAll(numericCode, kanjiCode);
      });
    }
  }
  return next;
